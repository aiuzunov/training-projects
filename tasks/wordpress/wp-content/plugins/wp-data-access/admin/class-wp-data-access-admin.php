...
use WPDataAccess\Addons\WPDA_Addons;
...
class WP_Data_Access_Admin {
	 ...
	 * @var string
	 */
	protected $title_submenu_addons;
	...
	public function enqueue_scripts() {
		...
		if ( self::PAGE_PUBLISHER === $this->page ) {
			...
			if ( WPDA::OPTION_DP_JSON_EDITING[1] === $json_editing ) {
				// Register codeEditorto support JSON editing in Data Publisher (table options advanced)
	...
	public function add_menu_items() {
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			// Add Addons page
//			$wpda_addons = new WPDA_Addons();
//			$this->wpda_data_designer_menu = add_submenu_page(
//				self::PAGE_MAIN,
//				$this->title_menu_menu,
//				$this->title_submenu_addons,
//				'manage_options',
//				self::PAGE_ADDONS,
//				[ $wpda_addons, 'show' ]
//			);
	...
	protected function set_menu_titles() {
		...
		$this->title_submenu_addons    = __( 'Addons', 'wp-data-access' );
	}
	...
	public function data_designer_page_not_found() {
		...
		<div class="wrap">
			<h1 class="wp-heading-inline">
				<span><?php echo $this->title_submenu_designer; ?></span>
			...
			<p>
				<?php echo __( 'ERROR: Repository table not found!', 'wp-data-access' ); ?>
			</p>
		</div>
		<?php
	}
	...
	public function data_publisher_page_not_found() {
		...
		<div class="wrap">
			<h1 class="wp-heading-inline">
				<span><?php echo $this->title_submenu_publisher; ?></span>
		...
		foreach ( WPDA_User_Menus_Model::list_external_menus() as $menu ) {
			...
			if ( $user_has_role ) {
				if ( ! isset( $menus_shown_to_current_user[ $menu->menu_slug . '/' . $menu->menu_name . '/' . $menu->menu_table_name . '/' . $menu->menu_schema_name ] ) ) {
					...
					$this->wpda_my_table_list_menu[ $menu_index ] =
						add_submenu_page(
							$menu->menu_slug,
					...
					$this->wpda_my_table_list_view[ $menu_index ] =
						new WPDA_List_View(
							[
