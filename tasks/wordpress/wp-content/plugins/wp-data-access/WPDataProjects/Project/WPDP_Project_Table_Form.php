		...
		protected function show_table_info() {
			...
			if ( '' === $html && '' === $html_child ) {
				...
				$hint =
					...
					' <span class="dashicons dashicons-editor-help" title="Only hyperlinks on list tables can be disabled"></span>';
			}

			if ( '' === $html ) {
				$html = __( 'No hyperlinks defined', 'wp-data-access' );
			}

			if ( '' === $html_child ) {
				$html_child = __( 'No hyperlinks defined', 'wp-data-access' );
			}
			?>
		...
		protected function show_relations() {
			...
			<script type='text/javascript'>
				...
				function add_column(index, selected_value = '') {
					...
					if (jQuery('#relation_type_' + index).val()==='lookup') {
						// Is lookup accessing a remote database?
						wpdp_get_columns(jQuery('#target_table_name_' + index).val(), index, '#target_column_name_' + index + '_' + eval(col_num[index] + 1), selected_value, jQuery('#target_schema_name_' + index).val());
					} else {
						wpdp_get_columns(jQuery('#target_table_name_' + index).val(), index, '#target_column_name_' + index + '_' + eval(col_num[index] + 1), selected_value);
					}
				...
				<fieldset class="wpda_fieldset">
					...
					<table class="wpda-table-structure">
						...
						<tr>
							...
							<th title="<?php echo __( 'Check to hide ID in lookup', 'wp-data-access' ) ?>">
