...
import cgi, cgitb, psycopg2, html
...
print(
    '<form name="userinputform" method="POST" action = "cgiscript.py"> <input name="name" type="text" pattern="[а-яА-Я \']*" placeholder = "Име на населено място" required /> </form>')
form = cgi.FieldStorage()
input = form.getvalue("name")
input = html.escape(input)
print("Вие търсихте населено място по име с ключовата дума: {:s}".format(input))
...
sql.execute(
    "SELECT selishta.ekatte, oblasti.name, obshtini.name, selishta.t_v_m, selishta.name FROM oblasti,obshtini,selishta WHERE LOWER(selishta.name) LIKE concat(LOWER(%s),%s) AND oblasti.oblast_id = obshtini.oblast_id AND obshtini.obshtina_id = selishta.obshtina_id ORDER BY selishta.ekatte;",
    (input, '%'))
...
for row in results:
    print("<tr> <td>", row[0], "</td> <td>", row[1], "</td> <td>", row[2], "</td> <td>", row[3], "</td> <td>", row[4],
          "</td> </tr>")
