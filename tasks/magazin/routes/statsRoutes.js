...
router.post("/soldProducts", async (req, res) => {
    try {
        ...
        var fromdate = from.split(",");
        var todate = to.split(",");
        const soldProducts = await pool.query("SELECT created AS DATE, SUM(quantity) AS Sales from order_items join orders on order_items.order_id = orders.id where DATE(created)>=$1 and DATE(created)<=$2 GROUP BY DATE",[fromdate[0],todate[0]]);
